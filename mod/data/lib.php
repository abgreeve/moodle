...
function data_refresh_events($courseid = 0, $instance = null, $cm = null) {
    ...
    if (isset($instance)) {
        ...
        if (isset($cm)) {
            if (!is_object($cm)) {
                $cm = get_coursemodule_from_instance('data', $instance->id, $instance->course);
            }
            $completionexpected = (!empty($cm->completionexpected)) ? $cm->completionexpected : null;
            \core_completion\api::update_completion_date_event($cm->id, 'data', $instance, $completionexpected);
        }
    ...
    foreach ($data as $datum) {
        $cm = get_coursemodule_from_instance('data', $datum->id, $datum->course);
        data_set_events($datum);
        $completionexpected = (!empty($cm->completionexpected)) ? $cm->completionexpected : null;
        \core_completion\api::update_completion_date_event($cm->id, 'data', $datum, $completionexpected);
    }
    return true;
}
