...
class remote_resource {
    ...
    public function download_to_requestdir(): array {
        ...
        // TODO: use a timeout controlled by a tool_moodlenet setting.
        //  Might also need a connection timeout.
        $downloadtimeout = 30;
    ...
    protected function get_resource_info() {
        ...
        $error_no = $this->curl->get_errno();
        $this->curl->resetopt();

        if ($error_no !== 0) {
            $message = 'Problem during HEAD request for remote resource \''.$this->url->get_value().'\'. Curl Errno: ' . $error_no;
