 ...
 * If courseid = 0, then every imscp event in the site is checked, else
 * only imscp events belonging to the course specified are checked.
 * This function is used, in its new format, by restore_refresh_events()
 *
 * @param int $courseid
 * @return bool
 */
function imscp_refresh_events($courseid = 0) {

    $courses = ($courseid == 0) ? get_courses() : array(get_course($courseid));
    foreach ($courses as $course) {

        $modinfo = get_fast_modinfo($course);
        $cms = $modinfo->instances['imscp'];

        foreach ($cms as $cm) {
            \core_completion\api::update_completion_date_event($cm->id, 'imscp', $cm->instance, $cm->completionexpected);
        }
    }
