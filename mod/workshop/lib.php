...
function workshop_refresh_events($courseid = 0, $instance = null, $cm = null) {
    ...
    if (isset($instance)) {
        ...
        if (isset($cm)) {
            if (!is_object($cm)) {
                $cm = get_coursemodule_from_instance('workshop', $instance->id, $instance->course);
            }
            workshop_calendar_update($instance, $cm->id);
            $completionexpected = (!empty($cm->completionexpected)) ? $cm->completionexpected : null;
            \core_completion\api::update_completion_date_event($cm->id, 'workshop', $instance, $completionexpected);
            return true;
        }
    }
    ...
    foreach ($workshops as $workshop) {
        ...
        $completionexpected = (!empty($cm->completionexpected)) ? $cm->completionexpected : null;
        \core_completion\api::update_completion_date_event($cm->id, 'workshop', $workshop, $completionexpected);
    }
    return true;
}
