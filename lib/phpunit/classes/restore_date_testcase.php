...
abstract class restore_date_testcase extends advanced_testcase {
    ...
    /**
     * Assert if properties of a module instances are rolled/not rolled as expected.
     *
     * @param stdClass $oldinstance
     * @param  stdClass $newinstance
     */
    protected function assertModuleDates($oldinstance, $newinstance) {
        // Properties that should not roll.
        $props = ['timecreated', 'timemodified'];
        foreach ($props as $prop) {
            // Zeros don't offset and we want to make sure these properties are not rolled.
            if (isset($oldinstance->$prop)) {
                $this->assertEquals($oldinstance->$prop, $newinstance->$prop, "'$prop' should not roll forward.");
            }
        }

        // Properties that should roll.
        $props = ['timeopen', 'timeclose', 'chattime', 'assesstimestart', 'assesstimefinish', 'timeavailablefrom',
                  'timeavailableto', 'timeviewfrom', 'timeviewto', 'editbegin', 'editend', 'allowsubmissionsfromdate',
                  'duedate', 'cutoffdate', 'submissionstart', 'submissionend', 'assessmentstart', 'assessmentend'];
        $diff = $this->get_diff();
        foreach ($props as $prop) {
            // Zeros don't offset and we want to make sure these properties are rolled.
            if (isset($oldinstance->$prop)) {
