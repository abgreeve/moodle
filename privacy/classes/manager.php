...
class manager {
    ...
    public function get_metadata_for_components() : array {
        ...
        foreach ($this->get_component_list() as $component) {
            if ($this->component_implements($component, \core_privacy\metadata\provider::class)) {
                $metadata[$component] = $this->get_provider_classname($component)::get_metadata(new item_collection($component));
            }
        }
        return $metadata;
    }
    ...
    public function get_contexts_for_userid(int $userid) : contextlist_collection {
        ...
        foreach ($this->get_core_provider_components() as $component) {
            $contextlist = $this->get_provider_classname($component)::get_contexts_for_userid($userid);
            $contextlist->set_component($component); // Each contextlist is tied to its respective component.
    ...
    public function export_user_data(contextlist_collection $contextlistcollection) {
        $approvedcontextlists = $contextlistcollection->get_contextlists();
        // Sanity checks.
        foreach ($approvedcontextlists as $approvedcontextlist) {
            ...
            // The component with which the contextlist is associated must be a core_data_provider of sorts.
            $component = $approvedcontextlist->get_component();
            if (!$this->component_is_core_provider($component)) {
                throw new \moodle_exception("Component '$component' must be a core data provider");
            }
        }
        ...
        foreach($approvedcontextlists as $approvedcontextlist) {
            ...
            if ($this->component_implements($component, \core_privacy\request\user_preference_provider::class)) {
                $this->get_provider_classname($component)::export_user_preferences($approvedcontextlist);
            }
        }
    ...
    public function delete_user_data(contextlist_collection $contextlistcollection) {
        ...
        foreach ($approvedcontextlists as $approvedcontextlist) {
            ...
            if (!$this->component_is_core_provider($approvedcontextlist->get_component())) {
                throw new \moodle_exception('Component must be a core provider');
            }
        }
        // Delete the data.
        foreach($approvedcontextlists as $approvedcontextlist) {
    ...
    /**
     * Returns all the components which are core providers.
     *
     * @return array the array of frankenstyle core provider component names.
     */
    protected function get_core_provider_components() {
        $cpcomponents = [];
        foreach ($this->get_component_list() as $component) {
            if ($this->component_implements($component, \core_privacy\request\core_data_provider::class)) {
                $cpcomponents[] = $component;
            }
        }
        return $cpcomponents;
    }
