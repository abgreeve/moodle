    ...
    public function __construct(assign $assignment,
        ...
        if ($this->assignment->get_instance()->teamsubmission) {
            $from .= 'LEFT JOIN ( -- Users with an assignment submission.
                          ...
                          FROM {user} u
                          INNER JOIN {assign_grades} g ON u.id = g.userid
                          ...
                          INNER JOIN (SELECT userid, max(attemptnumber) AS maxattempt
                               ...
                               GROUP BY userid) gs ON gs.userid = g.userid
                          LEFT JOIN {assign_submission} s ON s.attemptnumber = g.attemptnumber AND s.userid = g.userid
                                    AND s.assignment = :groupassignnumber2
                          WHERE g.assignment = :groupassignnumber3
                          AND gs.maxattempt = g.attemptnumber
                          UNION
                          -- Users with no assignment grades.
                          SELECT s.userid AS userid, s.status AS status, s.id AS submissionid,
                                s.timecreated AS firstsubmission, s.timemodified AS timesubmitted,
                                s.attemptnumber AS attemptnumber, g.id AS gradeid, g.grade AS grade,
                                g.timemodified AS timemarked, g.timecreated AS firstmarked
                            FROM {user} u
                          LEFT JOIN {assign_grades} g ON g.userid = u.id AND g.assignment = :groupassignnumber4
                          LEFT JOIN {assign_submission} s ON u.id = s.userid AND s.assignment = :groupassignnumber5
                          WHERE g.id is null) xxp ON xxp.userid = u.id ';
            ...
            $params['groupassignnumber4'] = (int)$this->assignment->get_instance()->id;
            $params['groupassignnumber5'] = (int)$this->assignment->get_instance()->id;
        } else {
