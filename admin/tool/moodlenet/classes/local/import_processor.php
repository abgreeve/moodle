...
class import_processor {
     ...
     *
     * @param url $url the URL to the remote file.
     * @param \stdClass $course the course object.
     * @param int $section the section, starting at 0.
     * @param string $modulename the module to pass the file to.
     * @param import_handler_registry $handlerregistry
     * @param \stdClass $user
     * @param remote_resource $remoteresource
     */
    public function __construct(
        ...
        // TODO: take only the reader as a param, not the file. We want to avoid needing to check the files in the reader match
        //  the file passed in.

        global $DB;
        $this->course = $course;
        require_login($this->course->id, false);
    ...
    public function process() {
        // First, make sure the module can handle this file extension.
        ...
        $supported = !empty(array_filter($handlers, function($handler) {
            return $handler->get_module_name() == $this->module->name;
        }));
        if (!$supported) {
            throw new \coding_exception("Module {$this->module->name} does not support extension '$extension'.");
        }
