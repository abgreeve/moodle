...
class restore_gradebook_structure_step extends restore_structure_step {
    ...
    protected const GRADE_CATEGORY_RECALCULATION_REQUIRED = 0;
    ...
    protected function process_grade_category($data) {
        ...
        if(empty($data->parent)) {
            ...
            $data->parent = self::GRADE_CATEGORY_RECALCULATION_REQUIRED;
        }
    ...
    protected function after_execute() {
        ...
        $select = 'courseid = :courseid AND (parent IS NULL OR parent = :categoryrecalculation)';
        $params = ['courseid' => $this->get_courseid(), 'categoryrecalculation' => self::GRADE_CATEGORY_RECALCULATION_REQUIRED];
        $parentcategories = $DB->get_records_select('grade_categories', $select, $params);

        $basecategory = null;
        $othercategories = [];
        foreach ($parentcategories as $key => $category) {
            $basecategory = (!isset($category->parent)) ? $category : $basecategory;
            if (isset($category->parent) && $category->parent == self::GRADE_CATEGORY_RECALCULATION_REQUIRED) {
                $othercategories[$key] = $category;
            }
        }
    ...
    /**
     * Sets the category parent for grade categories in this course. If the category is from the restored course then
     * it will use the new parent ID. If it is an existing category in the target and it is being merged then it will keep its
     * existing parent ID, and if the target is being overridden, then the target categories will be flagged as orphaned.
     *
     * @param stdClass $basecategory The first category that all other categories and grade items stem from.
     * @param stdClass[] $othercategories Other categories that have been marked to have the parent updated.
    ...
    protected function set_grade_category_parents(?stdClass $basecategory, array $othercategories): array {
        ...
        foreach ($recordset as $category) {
            if (!empty($category->parent)) {
                ...
                if (isset($othercategories[$parentid])) {
                ...
                } else if (empty($parentid) && $category->depth > 1) {
                    // These are existing categories that are either being over written or merged with the new ones. There will
                    // be no mapping for this category and so the parentid will be null or 0.
                    ...
                    // This category is re-using the same parent id it had before.
                    $gradecategory->parent = $category->parent;
                } else {
                    // These are new categories to be restored into the grade book. These take the new mapped parent id.
